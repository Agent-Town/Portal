...
| M1 Living V6 milestone contract | `in_progress` | Keep the V6 path explicit and refine it as progress lands. | This document, `specs/54_agent_town_v6_agent_civilization_foundation.md`, and `specs/release-gates/v60_agent_civilization_readiness_gate.md` cross-link and tests prove the major milestones remain named. |
| M2 V5 evidence promotion gates | `planned` | Promote V5.0-V5.5 from prototype evidence to release-grade prerequisites. | Each V5 slice has owner/auth checks, restart persistence tests, audit/replay evidence, security notes, and deterministic Node + Playwright coverage. |
...
| M4 Civic schema contracts | `planned` | Define the stable data contracts for V6 institutions. | Proposal, vote, delegation, civic action, reputation, moderation decision, rollback, and audit ledger schemas are documented and validated by contract tests. |
