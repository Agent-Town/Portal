...
auth, browser-session-bound CSRF, and IP/risk-aware production rate limits
remain promotion gates. Optional `WORLD_GRID_CSRF_SQLITE_PATH` coverage now
proves owner-bound hashed CSRF tokens persist across route restarts, and
optional `WORLD_GRID_RATE_LIMIT_SQLITE_PATH` coverage now proves owner/surface
mutation buckets persist across route restarts.
