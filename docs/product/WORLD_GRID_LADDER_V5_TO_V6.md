...
| V5.5 Controlled Free-Play Sandbox Districts | `prototype_gated` | No | Typed props, moderation rejection, agent demo, rollback, and private-town isolation coverage. |
| V6.0 Agent Civilization Foundation | `research_only` | No | Worker-first civic tool draft, reputation accountability foundation, moderation/privacy foundation, prepared-effect rollback foundation, scoped delegation foundation, civic institution charter foundation, public-works shared-resource foundation, modal lab surface contract, resilience baseline report, audit replay reconstruction, release-review gate, and controlled-release gate exist under `server/world_civilization/`, but runtime civic tools remain hidden until V5 safety, rollback, redaction, retention, modal-first observability, replay resilience, security/product signoff, controlled rollout controls, and civic-governance gates are proven. |
