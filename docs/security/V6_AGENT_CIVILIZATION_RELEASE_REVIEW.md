...
| Validation evidence | Targeted Node suite, split Playwright smokes, all-features regression, feature override safety, store-backed delegation proof, effect execution gate | Includes mutation security, vote approval policy, delegated-agent proof, governance preflight, and non-executing M11 effect execution gate contract coverage; pending release-candidate run |
