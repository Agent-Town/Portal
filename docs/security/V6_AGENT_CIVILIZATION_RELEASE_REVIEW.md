...
| Audit coverage | Append-only ledger, owner indexes, migration versions, replay reconstruction, rollback handles | Research-only replay reconstruction, v1 schema metadata drift checks, process restart probes for all current civic stores, audit-ledger load/rate replay evidence, and non-executing rollback recovery handle reconstruction exist; pending M16 release-grade replay, migration scripts, production load/rate, and real rollback recovery completion |
