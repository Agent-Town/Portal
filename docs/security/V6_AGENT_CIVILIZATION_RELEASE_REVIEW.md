...
| Validation evidence | Targeted Node suite, split Playwright smokes, all-features regression, feature override safety, store-backed delegation proof, effect execution gate, agent participation enforcement gate, institution readiness gate | Includes mutation security, vote approval policy, delegated-agent proof, governance preflight, non-executing M11 effect execution gate, non-executing M12 agent participation enforcement gate, and non-executing M13 institution readiness gate contract coverage; pending release-candidate run |
