...
| Audit coverage | Append-only ledger, owner indexes, migration versions, replay reconstruction, rollback handles | Research-only replay reconstruction plus process restart probes for all current civic stores exist; pending M16 release-grade replay, migration, load/rate, and rollback recovery completion |
