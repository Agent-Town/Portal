...
- Public release eligibility must come from the standalone `productionReleaseGate` report, which fails closed unless the gate uses a valid generated-pack id and every prerequisite, explicit approval, candidate review, human signoff, and non-future gate evaluation timestamp is present.
