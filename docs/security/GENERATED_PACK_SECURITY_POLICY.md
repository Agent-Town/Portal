...
- Release evidence bundles must be created at or after the bound release gate evaluation time, must not be future-dated relative to validation time, and must bind ready evidence sources, diversity-source pack inclusion/coherence, blocking reasons, and prerequisite snapshots back to the release gate; approval evidence drift or copied diversity reports from another suite fail validation even when supplied source hashes are stable.
