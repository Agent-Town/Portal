...
- Release-gate and release-evidence-bundle API requests must reject secret-like fields, secret-looking values, raw prompt instructions, and oversized/noisy evidence bodies before report construction; rejection responses may include counts, paths, and limits, but not submitted evidence values.
