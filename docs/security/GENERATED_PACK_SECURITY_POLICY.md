...
- Release-gate validation reports must mirror known release-prerequisite booleans/counts, bounded diversity/public-card/persistence/missing-asset/replayability evidence metrics, explicit auth/cost/consent, candidate-review binding, human-review, and production/private/default/V6 boundary indicators without exposing arbitrary submitted prerequisite keys or evidence identifiers. Release-evidence reports mirror production/private/default/V6 boundary indicators plus present, missing, required, and supplied source counts. They must redact unsafe submitted manifest hashes, evidence hashes, bundle hashes, pack ids, boundary values, and metric values before returning validation reports to callers.
