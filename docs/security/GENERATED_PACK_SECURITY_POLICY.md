...
- Public release eligibility must come from the standalone `productionReleaseGate` report, which fails closed unless the gate uses a valid generated-pack id and every prerequisite, generated-pack prerequisite source context, diversity evidence including the release pack, diversity metrics coherent with per-pack rows/signatures/screenshots/comparisons, same-pack persistence proof, same-pack public-card proof, explicit approval, candidate review, human signoff, non-future gate evaluation timestamp, ready source-evidence validation context, and prerequisite-evidence binding are present.
- Release approval evidence must be hash-bound, match the current generated pack id, contain timestamp-coherent approval events, include a nonzero accepted cost estimate, prove coherent candidate-review disposition counts, match the reviewed manifest's candidate-review counts, and remain candidate-only; stale hashes, future-dated approvals, zero-cost approval evidence, candidate reviews that predate their manifest, planned-only candidate approvals, candidate-review metric/row-status drift, candidate-review diagnostic metric tampering, approval-evidence/manifest candidate-count drift, incoherent candidate-review disposition counts, or evidence copied from another pack cannot unlock production readiness.
...
- Release evidence bundles must be created at or after the bound release gate evaluation time, must not be future-dated relative to validation time, and must bind ready evidence sources, source-count metrics, source-problem count metrics, source-presence/hash metrics, source-coverage metrics, release-gate validity metrics, release-gate source-context binding metrics, release-gate public-eligibility metrics, bundle/gate timing metrics, release-gate hash-match metrics, generated-pack-source validation, playtest-source validation, persistence-source validation, public-card-source validation, approval-evidence-source validation, candidate-review manifest source validation, candidate-review hash/time/count-match metrics, bounded approval-evidence/candidate-review diagnostic counts, diversity-source pack inclusion/coherence, blocking reasons, prerequisite snapshots, and production-boundary metrics back to the release gate; missing release-gate context, claimed hashes or pack ids for unsupplied sources, failing generated-pack sources, failing playtest sources, failing persistence sources, failing public-card sources, failing approval-evidence sources, failing candidate-review manifest sources, approval evidence drift, copied diversity reports from another suite, release-gate validity metric tampering, release-gate source-context binding metric tampering, release-gate public-eligibility metric tampering, bundle/gate timing metric tampering, gate-context metric tampering, ready-evidence aggregate metric tampering, primary source-pass metric tampering, review/diversity source metric tampering, production-boundary metric tampering, source-count metric tampering, source-problem count metric tampering, source-coverage metric tampering, source-presence metric tampering, release-gate hash metric tampering, candidate-review hash/time/count metric tampering, or approval-evidence/candidate-review diagnostic metric tampering fail validation even when supplied source hashes are stable.
- Release-gate validation reports must mirror known release-prerequisite booleans/counts, bounded diversity/public-card/persistence/missing-asset/replayability evidence metrics, bounded approval-evidence and candidate-review diagnostic counts, explicit auth/cost/consent, candidate-review binding, human-review, and production/private/default/V6 boundary indicators without exposing arbitrary submitted prerequisite keys, evidence identifiers, submitted evidence values, hashes, or candidate paths. Release-evidence reports mirror production/private/default/V6 boundary indicators, present/missing/required/supplied source counts, release-gate source-context binding metrics, and bounded approval-evidence/candidate-review diagnostic counts with scanner-safe metric names. They must redact unsafe submitted manifest hashes, evidence hashes, bundle hashes, pack ids, boundary values, and metric values before returning validation reports to callers.
...
```json
{
  ...
  "releaseEvidenceGateSourceContextMetricTamperRejected": true,
  ...
  "releaseEvidenceGateReadySourceEvidenceBound": true,
  "releaseEvidenceGatePrerequisiteEvidenceBound": true,
  ...
  "releaseEvidenceCandidateReviewCountMetricTamperRejected": true,
