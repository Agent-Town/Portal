...
- Release evidence report measured fields must mirror present, missing, required, and supplied source counts plus production/private/default/V6 boundary indicators, and must redact unsafe submitted manifest hashes, evidence hashes, bundle hashes, pack ids, boundary values, and metric values before returning validation reports to callers.
