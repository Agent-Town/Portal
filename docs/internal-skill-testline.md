...
| Founders Plot modal + foreman tool contract | `public/skill.md`, `public/app.js`, `server/founders_plot/*`, `vendors/openclaw-lite-main/src/openclaw-lite/gateway.js`, `vendors/openclaw-lite-main/src/openclaw-lite/worker.js` | Founders Plot stays modal-first in the town shell; worker-facing docs expose typed `founders_plot_*` aliases for plot, contract, standing-order, and scheduler tools plus the `founders-plot` modal open path; approvals remain explicit and deterministic while canonical server/replay ids stay dotted | `e2e/132_founders_plot_registry_handoff.spec.js`, `e2e/133_founders_plot_runtime_contract.spec.js`, `e2e/134_founders_plot_resume_recap.spec.js`, `e2e/145_founders_plot_v11_ui_surfaces.spec.js` |
| Founders Plot V1.2 living-town + worker-owned foreman loop contract | `public/skill.md`, `public/experiences/founders-plot/tools.md`, `public/experiences/founders-plot/goals.md`, `public/app.js`, `server/founders_plot/*`, `vendors/openclaw-lite-main/src/openclaw-lite/gateway.js`, `vendors/openclaw-lite-main/src/openclaw-lite/worker.js` | skill/docs name the V1.2 tool family with worker-safe aliases (`founders_plot_town_get_signals`, `founders_plot_town_upgrade_landmark`, `founders_plot_journal_get_entries`), recurring requesters, soft-deadline `PREPARATION` contracts, and the requirement that `Run now` executes through the OpenClaw Lite worker command path | `e2e/146_founders_plot_v12_worker_owned_tick.spec.js`, `e2e/147_founders_plot_v12_living_contract_deck.spec.js`, `e2e/148_founders_plot_v12_town_signals_and_journal.spec.js`, `e2e/149_founders_plot_v12_public_square_coin_sink.spec.js`, `e2e/150_founders_plot_v12_recap_and_ui.spec.js`, `e2e/55_phase3_skill_contract_line.spec.js` |
...
| Founders Plot real-LLM foreman + provider-safe worker tool naming | `specs/22_founders_plot_real_llm_foreman_and_provider_safe_tools.md`, `public/skill.md`, `public/experiences/founders-plot/skill.md`, `public/experiences/founders-plot/tools.md`, `server/founders_plot/*`, `vendors/openclaw-lite-main/src/openclaw-lite/worker.js` | real LLM tool calls use only provider-safe underscore names, docs make the worker/server contract split explicit, and `Run now` chooses through a bounded worker LLM selection turn instead of a fresh server-side chooser | `e2e/141_founders_plot_v11_standing_order_and_goal_arbitration.spec.js`, `e2e/156_worker_provider_safe_llm_tools_and_founders_foreman.spec.js`, `e2e/55_phase3_skill_contract_line.spec.js` |

## Progress Log

### 2026-04-20

- Added spec `23_real_llm_chat_validation_and_test_local_split.md` to document the headed-browser finding that plain chat already works with a real provider and that `pi-ai ok` specifically indicates the deterministic `test-local` chat stub path, not a generic chat failure.
...
- Added spec `22_founders_plot_real_llm_foreman_and_provider_safe_tools.md` plus deterministic coverage for provider-safe worker aliases and real-LLM Foreman selection (`e2e/141`, `e2e/156`, `e2e/55`).
