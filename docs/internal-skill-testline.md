...
| Worker+LLM ceremony + reconnect flow executes commit/reveal/connect actions with a single polling endpoint | `public/skill.md`, `server/index.js` | deterministic scripted LLM stream drives `agent_town_ceremony_commit`, `agent_town_ceremony_reveal`, and `http_request` routes using `/api/agent/state` for step polling plus `/api/agent/house/connect`; reconnect now requires the same browser session cookie for `houseId`-only calls or `teamCode` for external callers | `e2e/58_phase3_skill_playbook_behavior.spec.js` (`worker+llm skill run drives ceremony commit/reveal and house reconnect checks`), `e2e/118_security_regressions.spec.js` (`agent house reconnect rejects blind houseId mutation but still allows teamCode reconnect`) |
...
- Added deterministic worker+LLM scripted tool-call coverage for ceremony commit/reveal + single-endpoint `/api/agent/state` progression + `/api/agent/house/connect` reconnect behavior, plus regression coverage for same-session-vs-teamCode reconnect authorization.
