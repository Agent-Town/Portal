...
| House Library skill pack v2 exposes direct authoring, revisions, conversation capture, shelfing, and Public Stacks routing through worker-observable tools | `server/index.js`, `public/app.js`, `vendors/openclaw-lite-main/src/openclaw-lite/worker.js` | additive Library specialists use `house_library_update_item`, `house_library_read_revisions`, `house_library_capture_conversation`, `house_library_list_shelves`, `house_library_write_shelf`, `house_library_search_public_stacks`, and `house_library_preview_registry_artifact`; the page bridge remains available for deterministic Lite-tool execution and debug integration | `e2e/221_house_library_skill_tool_contract_v2.spec.js` (`M30.9: House Library skill pack v2 exposes Phase 30 tools and routes deterministically`) |
| House Library benchmark harness emits a deterministic measurable scorecard for AI agent developers | `server/platform_read_routes.js`, `server/index.js` | `POST /api/platform/library/benchmarks/run` reads active scope + prompt preview and returns stable metrics for scope precision, scope leakage, unsafe publish blocking, provenance visibility, and novice copy clarity | `e2e/222_house_library_benchmark_harness.spec.js` (`M30.10: House Library benchmark harness publishes a deterministic scorecard and stable hash`) |
| House Library guided memory loop keeps same-shell continuity across note authoring, conversation capture, shelves, Satchels, Public Stacks exchange, seal policy, and benchmark evaluation | `public/app.js`, `public/views/house.html`, `server/platform_read_routes.js` | one joined `/app` flow proves the late Library UX stays minimal, pixel-RPG readable, provenance-visible, and benchmark-clean without leaving the worker shell | `e2e/223_house_library_guided_memory_loop.spec.js` (`M30.11: House Library full guided memory loop stays same-shell and benchmark-clean`) |
...
- Added House Library Phase 30 late contract coverage for direct authoring/revisions/capture/shelves/Public Stacks routing (`e2e/221`), deterministic benchmark evaluation (`e2e/222`), and the full guided memory loop (`e2e/223`).
- Documented that the page debug bridge stays available for deterministic Lite-tool execution after reload, while the dedicated Worker Traffic pane contract continues to live in `e2e/53_agent_panel_global_presence.spec.js`.
