...
| Founders Plot ChatGPT-first Brain connection | `public/experiences/founders-plot/app.js`, `public/openclaw-lite/llm-config-library.js` | Foreman drawer leads with `Log in with ChatGPT`, completes the `openai-codex` PKCE exchange, saves the local brain config, and keeps provider/API-key setup under advanced options | `e2e/214_founders_plot_chatgpt_brain_login.spec.js` (`ChatGPT login is the primary brain CTA and persists openai-codex as Real Clover brain`) |
...
- Added the ChatGPT-first Clover budget row: Portal now exposes a sanitized Codex app-server `account/rateLimits/read` bridge, OpenClaw Lite stores local 5-hour/weekly game allocation, and Clover checks that budget before ChatGPT/Codex LLM turns.
- Added deterministic coverage for bridge-backed quota rendering, app-server-unavailable local fallback, budget persistence, pre-turn enforcement, and preservation of the alternate provider flow (`e2e/204`, `e2e/214`, `e2e/215`).
