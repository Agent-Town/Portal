...
- `release_approval_evidence.schema.json` is the auditable auth/cost/consent/candidate-review/human-review contract used by the release gate. It carries a stable `evidenceHash` and must match the generated pack id before it can unlock release prerequisites.
...
- Approval evidence or candidate-review manifests with secret-like fields, raw prompt instructions, hash drift, mixed pack ids, short candidate-review coverage, production image promotion, canonical rule changes, V6 civic changes, or default gameplay exposure fail closed.
