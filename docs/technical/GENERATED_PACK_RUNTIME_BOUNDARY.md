...
- Release-gate and release-evidence-bundle API ingress, including the generic tool dispatcher, rejects secret-like fields, secret-looking keys or values, raw executable prompt-instruction keys or values, oversized object keys, or oversized/noisy evidence bodies before constructing reports, so unsafe submitted values and oversized key text are not echoed back to callers.
