...
- `release_approval_evidence.schema.json` is the auditable auth/cost/consent/candidate-review/human-review contract used by the release gate. It carries a stable `evidenceHash`, requires timestamp-coherent approval events and a nonzero accepted cost estimate, and must match the generated pack id before it can unlock release prerequisites.
