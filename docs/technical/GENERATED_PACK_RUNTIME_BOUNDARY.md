...
- `POST /api/world/generated-pack/release-gate` and `et.world.generated_pack.release_gate` remain behind `FEATURE_WORLD_GRID_GENERATED_PACKS`; they can return a ready controlled-release report from complete machine, persistence, versioned approval, and candidate-review evidence, but do not change canonical world-grid rules, V6 civic systems, pack visibility, or production image policy.
