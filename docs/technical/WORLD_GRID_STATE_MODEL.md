...
| CSRF mutation tokens | Process-local `Map` in `server/world_grid/csrf.js`; optional SQLite `world_grid_csrf_tokens` table with `session_binding_hash` when `WORLD_GRID_CSRF_SQLITE_PATH` is configured | Durable foundation for owner-bound hashed tokens, hashed session binding when identity supplies a session id, expiry pruning, schema/migration versions, production route restart proof, and browser same-wallet cross-session denial proof; release promotion still needs final token invalidation/rotation policy and session-auth integration |
...
- CSRF-token and same-origin integration tests bound to the final
  ...
  cross-session token reuse denial with `WORLD_GRID_CSRF_REQUIRED=1`. Final
  token invalidation/rotation policy and session-auth integration remain gates.
