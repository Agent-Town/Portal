...
- CSRF-token and same-origin integration tests bound to the final
  ...
  browser coverage proves pre-reset tokens fail after `/api/session/reset`;
  final provider logout/disconnect invalidation wiring and session-auth
  integration remain gates.
