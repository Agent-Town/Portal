...
- Durable audit rows for every V5.1-V5.5 mutating route and tool surface with
  ...
  before/after region, territory, and preference summaries for every mutating
  route/tool surface. Complete store-specific before-state snapshots and release
  replay reconstruction remain gates.
...
- CSRF-token and same-origin integration tests bound to the final
  ...
  restarts; current Playwright coverage also proves same-wallet browser
  cross-session token reuse denial with `WORLD_GRID_CSRF_REQUIRED=1`. Current
  CSRF storage coverage proves same-session token rotation and explicit
  invalidation for process-local and optional SQLite stores, and current
  browser coverage proves pre-reset tokens fail after `/api/session/reset` and
  after `/api/session/world-grid-csrf/invalidate`, including the mocked provider
  disconnect callback path; final live Privy/provider logout signoff and
  session-auth integration remain gates.
