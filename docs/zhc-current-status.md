...
### M44.35 — placeholder inner share team line now reuses the saved HQ name
This pass stayed test-first and stayed deliberately tiny.

What changed:
- added a focused founders-loop assertion for the **placeholder** embedded public share team line on `/s/sh_missing`
- kept team line generic before HQ save (`human: … | agent: …`)
- branded it as `${saved HQ name} HQ • human: … | agent: …` after HQ save
- kept it to local/session placeholder preview flow only; no share payload or route changes

Implementation details:
- `e2e/457_zhc0_share_card_placeholder_team_line_uses_saved_hq_name.spec.js`
  - verifies placeholder team line is generic before HQ save on `/s/sh_missing?embed=1`
...
- added: `e2e/457_zhc0_share_card_placeholder_team_line_uses_saved_hq_name.spec.js`
  - verifies placeholder team line is generic before HQ save on a placeholder `/s/sh_missing?embed=1` path
  - verifies it becomes `${customName} HQ • human: … | agent: …` after HQ save on that placeholder path
  - verifies the placeholder badge remains `preview` so this remains the placeholder branch
  - verifies the branded placeholder team line survives reload and reopen
