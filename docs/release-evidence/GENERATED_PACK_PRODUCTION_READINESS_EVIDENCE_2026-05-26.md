...
| GU-19 release evidence bundle | `schemas/generated-packs/release_evidence_bundle.schema.json` and `tests/generated_pack_production_release_gate.test.js` bind release-gate reports to stable hashes and schema-bounded source pack ids, reject source drift, invalid source pack-id shapes, mixed-pack evidence, or missing ready-gate evidence, expose a generated-pack feature-gated QA endpoint, reject unsafe release API request bodies before echo, and keep production/default/V6 boundaries closed. | Covered as evidence integrity |
