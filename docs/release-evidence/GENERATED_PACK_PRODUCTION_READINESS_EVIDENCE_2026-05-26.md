...
| GU-19 release evidence bundle | `schemas/generated-packs/release_evidence_bundle.schema.json` and `tests/generated_pack_production_release_gate.test.js` bind release-gate reports to stable hashes, schema-bounded source pack ids, and candidate-review hash/time-order metrics; reject source drift, invalid source pack-id shapes, mixed-pack evidence, missing ready-gate evidence, or candidate-review time metric tampering; expose a generated-pack feature-gated QA endpoint; reject unsafe or oversized release API request bodies before echo; and keep production/default/V6 boundaries closed. | Covered as evidence integrity |
