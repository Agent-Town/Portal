...
| Mutation audit log | `server/world_grid/audit_log.js` writes hash-chained SQLite audit/replay rows for successful mutating V5.1+ routes/tools when `WORLD_GRID_AUDIT_SQLITE_PATH` is configured; route-level audit rows now include privacy-safe `agent-town.v5.world-grid.audit-snapshot.v1` before/after region, territory, and preference summaries; `tests/world_grid_audit_persistence.test.js` proves every V5.1-V5.5 mutating route/tool surface writes durable audit rows across separate Node process restarts, exact idempotent replay does not duplicate audit rows, changed-payload conflicts add no audit rows, before/after snapshots are present, and private-looking service secrets stay out of entries. Release promotion still needs complete store-specific before-state snapshots and release replay reconstruction. |
...
matrix coverage plus privacy-safe before/after route snapshots, optional SQLite
...
snapshots for every store, durable idempotency integration with final
