...
| Mutation CSRF guard | `server/world_grid/csrf.js` issues owner-bound tokens through `/api/world/mutation-token`; when the resolved runtime identity includes a session id, tokens are also bound to a hashed session binding; issuing a new token rotates old tokens for the same owner/session binding; `invalidateWorldGridCsrfTokens()` supports explicit session-token invalidation; `/api/session/reset` invalidates current-session world-grid CSRF tokens before rotating the human session cookie; `/api/session/world-grid-csrf/invalidate` invalidates the current owner/session token for wallet/provider disconnect cleanup and `public/app.js` calls it before clearing local wallet identity; production mutating routes/tools reject missing, invalid, cross-owner, cross-session, and expired tokens; `tests/world_grid_csrf_persistence.test.js` proves optional SQLite `WORLD_GRID_CSRF_SQLITE_PATH` token/session hashes survive reopen, reject cross-session reuse, rotate and invalidate old same-session tokens, and authorize production mutating routes across separate Node process restarts; `e2e/243_world_grid_csrf_session_binding.spec.js` proves browser same-session token rotation, same-wallet cross-session token reuse denial, pre-reset token rejection after session reset, old-token rejection after wallet/provider disconnect invalidation, and old-token rejection after the mocked provider disconnect callback path while a new same-session token succeeds under `WORLD_GRID_CSRF_REQUIRED=1`. |
| Mutation rate-limit guard | `server/world_grid/rate_limit.js` applies owner/surface buckets to mutating world-grid routes/tools and returns `RATE_LIMITED` with retry headers when exceeded; exact idempotent replays are detected before rate-limit consumption and return `x-world-grid-idempotency-replay: 1` without consuming another bucket hit, while changed-payload idempotency conflicts still pass through the limiter; `tests/world_grid_rate_limit_persistence.test.js` proves optional SQLite `WORLD_GRID_RATE_LIMIT_SQLITE_PATH` counters survive separate Node process restarts, block mutating routes after restart, and hash optional `WORLD_GRID_RATE_LIMIT_IDENTITY_MODE=owner_session_ip_risk` session/IP/risk bucket identity without storing raw session/IP values; `server/world_grid/rate_limit_rollout.js` and `tests/world_grid_rate_limit_rollout.test.js` keep trusted proxy/risk-signal production rollout, distributed counters, per-surface calibration, abuse-burst backoff, and production observability as explicit non-executing release gates. |
| Mutation audit log | `server/world_grid/audit_log.js` writes hash-chained SQLite audit/replay rows for successful mutating V5.1+ routes/tools when `WORLD_GRID_AUDIT_SQLITE_PATH` is configured; route-level audit rows now include privacy-safe `agent-town.v5.world-grid.audit-snapshot.v1` before/after region, territory, preference, public presence, services, events, and sandbox aggregate summaries; `tests/world_grid_audit_persistence.test.js` proves every V5.1-V5.5 mutating route/tool surface writes durable audit rows across separate Node process restarts, exact idempotent replay does not duplicate audit rows, changed-payload conflicts add no audit rows, before/after snapshots are present with store-specific aggregate deltas, and private-looking service secrets stay out of entries. `server/world_grid/replay_reconstruction.js` and `tests/world_grid_replay_reconstruction.test.js` now verify current audit hash-chain replay, privacy-safe snapshots, migration metadata, rollback counts, and aggregate summary coverage without applying world state while keeping `releaseReplayReady: false`. Release promotion still needs complete exact per-record before-state reconstruction and release-grade replay reconstruction. |
...
| Player-route prerequisite proof | `e2e/245_world_grid_player_route_prerequisite.spec.js` proves a same-session V5.1 claim mutation returns `WORLD_GRID_PLOT_REQUIRED` and creates no Founders Plot public row before the player opens `/app?district=founders-plot&entry=play-first`; after the app-created Founders Plot has an HQ, the same browser session can plan a World Grid claim while V6 civic runtime tools remain hidden. |
...
matrix coverage plus privacy-safe before/after route snapshots with store-specific
public presence, services, events, and sandbox aggregate summaries, optional SQLite
CSRF rows now have owner-bound hashed-token and
hashed session-binding route restart coverage, browser coverage now proves
same-wallet cross-session CSRF denial, CSRF storage coverage now proves
same-session token rotation and explicit invalidation, session reset now
invalidates current-session world-grid tokens, wallet/provider disconnect
cleanup now has an explicit invalidation endpoint plus client hook and mocked
provider disconnect callback proof, and optional SQLite rate-limit rows now have
owner/surface route restart coverage, replay-safe exact idempotent retries, and
hashed owner/session/IP/risk bucket identity coverage. Changed-payload
idempotency conflicts still pass through the limiter. Release promotion still
requires durable owner indexes, migration versioning, complete exact
per-record before-state reconstruction for every store, durable idempotency
integration with final session-auth production replay, live Privy/provider logout signoff,
trusted proxy/risk-signal production rollout, and release replay reconstruction
for every world-grid store. `server/world_grid/rate_limit_rollout.js` documents
the current trusted proxy/risk-signal rollout target only; it does not make the
process-local or optional SQLite prototype rate-limit foundation release-grade.
