...
| Delegated worker action budgets needed real accounting | The internal worker proposal and vote adapters now consume scoped delegated action budget idempotently for successful receipts. Exact replays return the existing `delegation.action_consumed` usage row, while distinct proposal/vote attempts fail once the delegation budget is exhausted and do not create additional receipts. | This remains internal research-only evidence; browser worker/runtime registration, broader route-edge authorization, delegated effect execution, and release signoff remain gates. |
