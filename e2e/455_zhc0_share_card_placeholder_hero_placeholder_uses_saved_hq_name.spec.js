...
test('M44.33 placeholder inner share hero placeholder stays generic before HQ save and brands after HQ save', async ({ page, request }) => {
  ...
  const heroPlaceholder = shareFrame.getByTestId('share-hero-placeholder');
  ...
  await expect(heroPlaceholder).toHaveText('Generated house hero will appear here once the public share card is minted.');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', /\/s\/sh_missing\?embed=1$/);
  await expect(heroPlaceholder).toHaveText(`${customName} HQ hero will appear here once the public share card is minted.`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText('preview');
