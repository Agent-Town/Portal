...
test('M44.4 create crest contract stays blocked until ink exists and keeps House hidden before completion', async ({ page }) => {
  const port = randomPort();
  const baseUrl = `http://127.0.0.1:${port}`;
  const storePath = path.join(
    os.tmpdir(),
    `agent-town-zhc0-create-crest-${Date.now()}-${Math.random().toString(16).slice(2)}.sqlite`
  );
  ...
  try {
    ...
    const crestRoot = page.getByTestId('zhc-create-root');
    await expect(crestRoot).toBeVisible();
    await expect(crestRoot).toHaveAttribute('data-zhc-phase', 'alignment_passed');
    await expect(crestRoot).toHaveAttribute('data-zhc-overlay-state', 'blocked');
    await expect(crestRoot).toHaveAttribute('data-zhc-progress-step', '4');
    await expect(crestRoot).toHaveAttribute('data-zhc-progress-total', '9');
    await expect(crestRoot).toHaveAttribute('data-zhc-blocker-key', 'needs_crest');
    await expect(crestRoot).toHaveAttribute('data-zhc-next-unlock', 'house');

    const shareBtn = page.getByTestId('share-btn');
    await expect(shareBtn).toBeDisabled();
    await expect(shareBtn).toHaveAttribute('data-zhc-primary-action', 'true');
    ...
    await expect(crestRoot).toHaveAttribute('data-zhc-overlay-state', 'ready');
    const blockerKey = await crestRoot.getAttribute('data-zhc-blocker-key');
