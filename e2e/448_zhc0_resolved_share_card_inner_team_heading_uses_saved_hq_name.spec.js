...
test('M44.26 resolved inner share team panel heading stays generic before HQ save and brands itself after HQ save', async ({ page, request }) => {
  ...
  await expect(shareFrame.getByTestId('share-team-panel-title')).toHaveText('👤 Team');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', resolvedSrc);
  await expect(shareFrame.getByTestId('share-team-panel-title')).toHaveText(`👤 ${customName} HQ Team`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText(share.shareId);
