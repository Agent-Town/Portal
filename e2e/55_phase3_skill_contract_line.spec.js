...
test('skill.md keeps the minimal external-agent contract', async ({ request }) => {
  ...
  expect(txt).toContain('founders_plot_get_state');
  expect(txt).toContain('founders_plot_place_building');
  expect(txt).toContain('founders_plot_collect_outputs');
  expect(txt).toContain('founders_plot_request_user_approval');
  expect(txt).toContain('founders_plot_town_get_signals');
  expect(txt).toContain('founders_plot_town_upgrade_landmark');
  expect(txt).toContain('founders_plot_journal_get_entries');
  expect(txt).toContain('founders_plot_contracts_get_state');
  expect(txt).toContain('founders_plot_contracts_accept');
  expect(txt).toContain('founders_plot_contracts_turn_in');
  expect(txt).toContain('founders_plot_foreman_policy_set_standing_order');
  expect(txt).toContain('founders_plot_foreman_scheduler_enable_collect_ready_outputs');
  expect(txt).toContain('Worker-facing aliases use underscore names; server/API/replay routes remain dotted `et.plot.*` and `et.foreman.*` ids.');
  ...
  expect(txt).toContain('Use `founders_plot_town_get_signals` and `founders_plot_journal_get_entries` to explain how the town changed.');
