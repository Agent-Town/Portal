...
const { reset, uploadBytes } = require('./helpers/static_asset');

test.beforeEach(async ({ request }) => {
  await reset(request);
});

test('static-asset endpoints have stable contract shapes', async ({ request }) => {
  // Missing image
  const missing = await request.post('/api/static-asset/upload', { data: {} });
  ...
  // Unknown job
  const jobResp = await request.get('/api/static-asset/jobs/asj_does_not_exist');
