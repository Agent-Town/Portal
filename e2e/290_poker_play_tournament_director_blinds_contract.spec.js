...
test('M25.x: director can queue one blind increase behind a live hand and the next hand starts at the higher level', async ({ browser, request }) => {
  const sitAt = '2026-03-12T10:00:00.500Z';
  const advanceAt = '2026-03-12T10:00:00.900Z';
  const foldAt = '2026-03-12T10:00:01.000Z';
  const userA = {
    address: 'So1anaMockTourA111111111111111111111111111111',
    houseId: 'house_director_blind_a',
    streamId: 'stream-director-blind-a',
  };
  const userB = {
    address: 'So1anaMockTourB111111111111111111111111111111',
    houseId: 'house_director_blind_b',
    streamId: 'stream-director-blind-b',
  };
  ...
  expect(Number(detail?.table?.summary?.pendingBlindAdvanceCount || 0)).toBe(0);
  expect(Number(detail?.table?.summary?.upcomingBlindLevel || 0)).toBe(1);
  expect(Number(detail?.table?.smallBlindOil || 0)).toBe(50);
  expect(Number(detail?.table?.bigBlindOil || 0)).toBe(100);

  let adminResp = await request.post('/api/poker/play/admin/tables/pkt_play_tournament_01/blinds/advance', {
    headers: ADMIN_HEADERS,
    data: {
      reason: 'Director jumped the next blind level.',
      asOf: advanceAt,
    },
  });
  expect(adminResp.ok()).toBe(true);
  let adminBody = await adminResp.json();
  expect(Number(adminBody?.data?.table?.summary?.blindLevel || 0)).toBe(1);
  expect(Number(adminBody?.data?.table?.summary?.pendingBlindAdvanceCount || 0)).toBe(1);
  expect(Number(adminBody?.data?.table?.summary?.upcomingBlindLevel || 0)).toBe(2);
  expect(Number(adminBody?.data?.hand?.blindLevel || 0)).toBe(1);

  detail = await getTable(pageA, userA.address, { asOf: advanceAt });
  expect(Number(detail?.table?.summary?.blindLevel || 0)).toBe(1);
  expect(Number(detail?.table?.summary?.pendingBlindAdvanceCount || 0)).toBe(1);
  expect(Number(detail?.table?.summary?.upcomingBlindLevel || 0)).toBe(2);
  expect(Number(detail?.table?.smallBlindOil || 0)).toBe(50);
  expect(Number(detail?.table?.bigBlindOil || 0)).toBe(100);
  expect(Number(detail?.hand?.blindLevel || 0)).toBe(1);

  adminResp = await request.get('/api/poker/play/admin/tables/pkt_play_tournament_01/review?asOf=2026-03-12T10%3A00%3A00.900Z', {
    headers: ADMIN_HEADERS,
  });
  expect(adminResp.ok()).toBe(true);
  adminBody = await adminResp.json();
  const blindAudit = (adminBody?.data?.auditEvents || []).filter((event) => event?.eventKind === 'director_blinds_advanced');
  expect(blindAudit).toHaveLength(1);
  expect(blindAudit[0]?.payload?.appliedOnNextHand).toBe(true);
  expect(Number(blindAudit[0]?.payload?.previousBlindLevel || 0)).toBe(1);
  expect(Number(blindAudit[0]?.payload?.nextBlindLevel || 0)).toBe(2);
  ...
  expect(Number(detail?.table?.summary?.pendingBlindAdvanceCount || 0)).toBe(0);
  expect(Number(detail?.table?.summary?.upcomingBlindLevel || 0)).toBeGreaterThanOrEqual(2);
