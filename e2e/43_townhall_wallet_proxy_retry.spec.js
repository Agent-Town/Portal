...
async function installWalletProxyRetryMocks(page) {
  ...
  await page.addInitScript(({ evmAddress: evmAddr, solAddress: solAddr }) => {
    ...
    window.__PRIVY_WALLET_BRIDGE__ = {
      ...
      connectEvm: async () => {
        evmConnectAttempts += 1;
        if (evmConnectAttempts === 1) {
          throw new Error('EMBEDDED_WALLET_PROXY_NOT_INITIALIZED');
        }
        if (evmConnectAttempts === 2) {
          throw new Error('EMBEDDED WALLET PROXY NOT INITIALIZED');
        }
        return { address: evmAddr, provider: evmProvider };
      },
      disconnectEvm: async () => {},
...
test('town hall registration retries Privy embedded wallet proxy initialization', async ({ page }) => {
  await installWalletProxyRetryMocks(page);
  await page.goto('/app');
