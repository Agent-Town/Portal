...
test('M44.36 placeholder inner share lead stays generic before HQ save and brands after HQ save', async ({ page, request }) => {
  ...
  const shareLead = shareFrame.getByTestId('share-lead');
  ...
  await expect(shareLead).toHaveText('Placeholder shell for a house whose public share card is still offline.');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', /\/s\/sh_missing\?embed=1$/);
  await expect(shareLead).toHaveText(`Placeholder shell for ${customName} HQ while the public share card is still offline.`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText('preview');
