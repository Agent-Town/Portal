...
test('M44.28 resolved inner share primary action stays generic before HQ save and brands itself after HQ save', async ({ page, request }) => {
  ...
  const brandedLabel = `Join ${customName} HQ`;
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', resolvedSrc);
  await expect(signupButton).toHaveText(brandedLabel);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText(share.shareId);
