...
async function uploadFixture(request, name, { headers } = {}) {
  const p = fixturePath(name);
  const buf = fs.readFileSync(p);
  const resp = await request.post('/api/avatar/upload', {
    headers,
    multipart: {
      avatar: {
        name,
...
async function uploadBytes(request, { filename, mimeType, buffer, headers } = {}) {
  const resp = await request.post('/api/avatar/upload', {
    headers,
    multipart: {
      avatar: {
...
async function waitForTerminalJob(request, jobId, timeoutMs = 25000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const r = await request.get(`/api/avatar/jobs/${encodeURIComponent(jobId)}`);
