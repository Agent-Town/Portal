...
test('M44.25 resolved inner share hero panel heading stays generic before HQ save and brands itself after HQ save', async ({ page, request }) => {
  ...
  await expect(shareFrame.getByTestId('share-hero-panel-title')).toHaveText('🎨 House Hero');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', resolvedSrc);
  await expect(shareFrame.getByTestId('share-hero-panel-title')).toHaveText(`🎨 ${customName} HQ Hero`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText(share.shareId);
