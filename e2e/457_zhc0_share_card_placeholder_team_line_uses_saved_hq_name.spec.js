...
test('M44.35 placeholder inner share team line stays generic before HQ save and brands after HQ save', async ({ page, request }) => {
  ...
  await expect(teamLine).toHaveText('human: … | agent: …');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', /\/s\/sh_missing\?embed=1$/);
  await expect(teamLine).toHaveText(`${customName} HQ • human: … | agent: …`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText('preview');
