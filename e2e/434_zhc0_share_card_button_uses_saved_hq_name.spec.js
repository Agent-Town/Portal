...
test('M44.12 saved HQ name brands the reconnect share-card button after save and reload', async ({ page, request }) => {
  ...
  await expect(shareCardButton).not.toContainText(customName);
  await expect(shareCardButton).not.toContainText('HQ share card');

  await input.fill(customName);
  await primary.click();

  await expect(page.getByTestId('house-experiences-panel')).toBeVisible();
  await expect(shareCardButton).toHaveText(/^(Open|Preview) Shared Orbit HQ share card$/);
