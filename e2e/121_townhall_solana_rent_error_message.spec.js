...
test('town hall keeps Solana rent funding failures on the Solana path', async ({ page }) => {
  ...
  const rentError = [
    'Simulation failed.',
    'Message: Transaction simulation failed: Transaction results in an account (2) with insufficient funds for rent.'
  ].join(' ');
  ...
  await expect(error).toContainText('Solana sponsored send needs more temporary lamports for rent-exempt account creation.');
  await expect(error).not.toContainText('Sepolia wallet has insufficient ETH for gas.');
