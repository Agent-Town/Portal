...
test('M44.31 placeholder inner share team heading stays generic before HQ save and brands after HQ save', async ({ page, request }) => {
  ...
  const teamPanelTitle = shareFrame.getByTestId('share-team-panel-title');
  ...
  await expect(teamPanelTitle).toHaveText('👤 Team');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', /\/s\/sh_missing\?embed=1$/);
  await expect(teamPanelTitle).toHaveText(`👤 ${customName} HQ Team`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText('preview');
