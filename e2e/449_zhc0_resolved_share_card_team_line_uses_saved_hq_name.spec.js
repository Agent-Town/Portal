...
test('M44.27 resolved inner share team line stays generic before HQ save and brands itself after HQ save', async ({ page, request }) => {
  ...
  const teamLine = shareFrame.getByTestId('team-line');
  ...
  await expect(teamLine).toContainText('human:');
  await expect(teamLine).toContainText('agent:');
  const genericTeamLine = (await teamLine.innerText()).trim();
  expect(genericTeamLine.includes(customName)).toBeFalsy();
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', resolvedSrc);
  await expect(teamLine).toHaveText(`${customName} HQ • ${genericTeamLine}`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText(share.shareId);
