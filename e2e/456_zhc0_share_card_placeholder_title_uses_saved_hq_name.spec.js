...
test('M44.34 placeholder inner share title stays generic before HQ save and brands after HQ save', async ({ page, request }) => {
  ...
  const shareCardTitle = shareFrame.getByTestId('share-card-title');
  ...
  await expect(shareCardTitle).toHaveText('Share Card');
  ...
  await shareCardButton.click();
  await expect(shareFrameHost).toHaveAttribute('src', /\/s\/sh_missing\?embed=1$/);
  await expect(shareCardTitle).toHaveText(`${customName} HQ share card`);
  await expect(shareFrame.locator('#shareIdBadge')).toHaveText('preview');
