...
Release-gate API ingress, including the generic tool dispatcher, now rejects secret-like fields, raw executable prompt-instruction text, and oversized/noisy evidence bodies before building or echoing release reports. Rejection responses include counts, limits, and safe field paths only, not submitted secret, instruction, or evidence values.
