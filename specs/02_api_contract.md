...
- missing canonical contracts, unknown contract/action IDs, hidden mechanics, reward deltas, formulas, mutation authority, secret-like fields including semantic token fields, raw executable prompt instructions, and provider/debug jargon are rejected;
...
Imports a generated-pack export into the current owner session. The server rejects tampered hashes, invalid generated-pack content, private owner fields, raw prompt fields, raw-instruction keys, secret-like fields, secret-looking keys or values, and forbidden authority fields before saving. Final generated-pack validation reports redact submitted secret-looking and raw-instruction key text from content and schema-error paths.
...
- `releaseGate.packId`, which must match the generated-pack id shape `gen_pack_<16 hex>`;
...
- `releaseGate.metrics.diversityPackIdMatches`;
...
- complete machine, persistence, versioned approval, and candidate-review evidence can produce `ready-for-controlled-release` HTTP/tool responses while production image creation, default generated-pack exposure, canonical server-rule changes, V6 civic changes, and private-data storage remain false/zero;
- `validationReport.metrics` mirrors direct release-gate private-data, production-image, default-exposure, canonical-rule, and V6 boundary indicators so HTTP/tool callers can classify boundary failures without recomputing gate internals;
- `validationReport.metrics` mirrors direct release-gate auth/cost/consent, candidate-review binding, human-review, approval-input/evidence matching, ready-source evidence binding, and prerequisite-evidence binding indicators so HTTP/tool callers can classify approval and source-context failures without trusting loose approval inputs, metric-only ready/prerequisite claims, or generated-pack prerequisite claims without source context;
- `validationReport.metrics` mirrors known direct release-gate prerequisite booleans and prerequisite counts, but must not echo arbitrary submitted prerequisite keys;
- `validationReport.metrics` mirrors bounded direct release-gate diversity/public-card/persistence/missing-asset/replayability evidence indicators, but must not echo submitted evidence identifiers or source hashes;
- `validationReport.metrics` mirrors bounded approval-evidence and candidate-review diagnostic counts, but must not echo submitted evidence values, hashes, or candidate paths;
- `evaluatedAtMs` must not be future-dated relative to validation time;
- `publicReleaseEligible=true` requires all prerequisites to be true, `blockingReasons=[]`, diversity evidence including the release pack, diversity aggregate metrics that match per-pack rows/signatures/screenshots/comparisons, same-pack persistence evidence, same-pack public-card evidence, source evidence context for ready-gate validation, zero private-data leaks, zero missing assets, zero production image assets, explicit consent/cost/auth approval, candidate asset review, and a 64-hex human review signoff hash;
- forged eligibility, mismatched blocking reasons, invalid gate pack ids, diversity evidence that excludes the release pack, metric-only diversity claims, metric-only ready source claims, metric-only prerequisite claims, generated-pack prerequisite claims without source context, mixed persistence evidence, mixed public-card evidence, approval evidence hash drift, future-dated approval evidence, future-dated gate reports, candidate-review approval evidence that predates its manifest, approval-evidence/manifest candidate-count drift, candidate-review diagnostic metric tampering, planned-only candidate-review approvals, approval evidence copied from another pack, secret-like fields including semantic token fields, raw prompt instructions, production asset leaks, and private-card data fail validation;
...
- the generic tool dispatcher path `et.world.generated_pack.release_gate` uses the same generated-pack guard and release-gate contract as the HTTP endpoint;
...
- `releaseEvidenceBundle.metrics.requiredSourceCount`, `presentSourceCount`, and `missingSourceCount`;
- `releaseEvidenceBundle.metrics.sourceHashMismatchCount` and `sourcePackIdMismatchCount`;
...
- `releaseEvidenceBundle.metrics.releaseGateValid`;
- `releaseEvidenceBundle.metrics.releaseGateReadySourceEvidenceBound`;
- `releaseEvidenceBundle.metrics.releaseGatePrerequisiteEvidenceBound`;
- `releaseEvidenceBundle.metrics.releaseGatePublicEligible`;
...
- `releaseEvidenceBundle.metrics.generatedPackSourcePassed`;
- `releaseEvidenceBundle.metrics.playtestSourcePassed`;
- `releaseEvidenceBundle.metrics.persistenceSourcePassed`;
- `releaseEvidenceBundle.metrics.publicCardSourcePassed`;
...
- `releaseEvidenceBundle.metrics.approvalEvidenceHashMatchesGate`;
...
- `releaseEvidenceBundle.metrics.approvalEvidenceSchemaErrorCount`, `approvalEvidenceSensitiveFieldCount`, and `approvalEvidenceRawInstructionCount`;
- `releaseEvidenceBundle.metrics.candidateReviewManifestSchemaErrorCount`, `candidateReviewManifestSensitiveFieldCount`, `candidateReviewManifestRawInstructionCount`, `candidateReviewExpectedTargetCount`, and `candidateReviewCoverageCount`;
- `releaseEvidenceBundle.metrics.productionImageAssetCount` and `privateDataLeakCount`;
- `validationReport.metrics` mirrors source counts (`presentSourceCount`, `missingSourceCount`, `requiredSourceCount`, `suppliedSourceCount`), source-pass booleans, production/private/default/V6 boundary indicators (`productionImageAssetCount`, `privateDataLeakCount`, `productionImageAssetsCreated`, `externalProviderPrivateDataStored`, `canonicalServerRulesChanged`, `v6CivicMechanicsTouched`, `normalGameplayVisibilityChanged`, `generatedPackDefaultExposure`, `boundaryPreserved`), approval-evidence hash binding, release-gate validity, release-gate source-context binding, release-gate public eligibility, bundle/gate timing booleans, gate-context binding booleans, ready-evidence aggregate boolean, and bounded approval-evidence/candidate-review diagnostic counts above so QA can classify omitted bundle evidence, omitted validation inputs, generated-pack, playtest, persistence, public-card, diversity, approval-evidence, candidate-review manifest, release-gate validity, release-gate source-context, release-gate eligibility, timestamp-coherence, blocking-reason, prerequisite-snapshot, boundary, evidence-diagnostic, and ready-source failures without inspecting internal checks or submitted evidence values, hashes, or candidate paths;
- `validationReport`, which rejects missing release-gate context, source drift, claimed hashes or pack ids for unsupplied sources, mixed pack ids, failing generated-pack sources, failing playtest sources, failing persistence sources, failing public-card sources, failing approval-evidence sources, failing candidate-review manifest sources, copied diversity reports that exclude the release pack, invalid source pack-id shapes, bundles created before their bound gate, future-dated bundles, forged blocking reasons, forged prerequisite snapshots, approval evidence drift from the bound gate, missing ready-gate evidence, release-gate validity metric tampering, release-gate source-context binding metric tampering, release-gate public-eligibility metric tampering, bundle/gate timing metric tampering, gate-context metric tampering, ready-evidence aggregate metric tampering, primary source-pass metric tampering, review/diversity source metric tampering, production-boundary metric tampering, source-count metric tampering, source-problem count metric tampering, source-coverage metric tampering, source-presence metric tampering, release-gate hash metric tampering, candidate-review hash/time/count metric tampering, approval-evidence/candidate-review diagnostic metric tampering, unsafe request content, and boundary violations.
...
- the generic tool dispatcher path `et.world.generated_pack.release_evidence_bundle` uses the same guard, returns the same fail-closed rejection code, and exposes the same validation-report source-count metrics, approval-evidence/candidate-review diagnostic counts, and production/private/default/V6 boundary indicators as the HTTP endpoint;
