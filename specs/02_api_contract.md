...
- `validationReport.metrics` mirrors direct release-gate auth/cost/consent, candidate-review binding, human-review, approval-input/evidence matching, and ready-source evidence binding indicators so HTTP/tool callers can classify approval and source-context failures without trusting loose approval inputs or metric-only ready claims;
