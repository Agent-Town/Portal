...
- request bodies share the release-gate ingress guard for secret-like fields, secret-looking keys or values, raw prompt instructions, request depth, node count, array size, object key count, and string length;
...
- rejection responses must not echo raw submitted secrets, secret-looking keys or values, prompt instructions, or oversized evidence values.
