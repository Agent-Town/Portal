...
async function runHouseLibraryCaptureConversation(params, toolName = "house_library_capture_conversation") {
  ...
  const idempotencyKey = typeof params?.idempotencyKey === "string" && params.idempotencyKey.trim()
    ? params.idempotencyKey.trim()
    : randomId("house_library_capture");
  const body = {
    title: typeof params?.title === "string" ? params.title.trim() : "",
...
async function runHouseLibraryPreviewRegistryArtifact(params, toolName = "house_library_preview_registry_artifact") {
  const startedAtMs = nowMs();
  const registryEntityId = typeof params?.registryEntityId === "string"
    ? params.registryEntityId.trim()
    : (typeof params?.registryId === "string" ? params.registryId.trim() : "");
  if (!registryEntityId) {
