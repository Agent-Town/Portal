    ...
    const errorCode =
      typeof data?.error === "string"
        ...
        : typeof data?.error?.code === "string"
          ? data.error.code
          : `HTTP_${res.status}`;
    const err = new Error(String(errorCode || `HTTP_${res.status}`));
    err.payload = data;
...
function makeFoundersPlotIdempotencyKey(toolName = "et.plot.action") {
  const normalized = String(toolName || "et.plot.action")
    .trim()
    .toLowerCase()
...
async function runFoundersPlotTool(params, toolName) {
  ...
  const safeParams = normalizedToolName === "et.plot.get_state"
    ? (isPlainObject(params) ? { ...params } : {})
    : withFoundersPlotIdempotency(params, normalizedToolName);

  try {
    const payload = normalizedToolName === "et.plot.get_state"
      ? await apiJson("/api/founders-plot/state", { method: "GET" })
      : await apiJson(`/api/founders-plot/tool/${encodeURIComponent(normalizedToolName)}`, {
        method: "POST",
        body: JSON.stringify({
          ...safeParams,
        }),
    ...
    const stateSnapshot =
      normalizedToolName === "et.plot.get_state"
        ? payload?.state || null
    ...
    const retryable = typeof errorNode?.retryable === "boolean"
      ? errorNode.retryable
      : code === "RATE_LIMITED" || code === "SIMULATION_DESYNC" || code === "SERVER_ERROR";
...
async function foundersPlotWorkerFetch(path, { method = "GET", token = "", body = null } = {}) {
  const headers = {};
...
const FOUNDERS_PLOT_SCHEDULER_MIN_DELAY_MS = 250;
const FOUNDERS_PLOT_SCHEDULER_MAX_DELAY_MS = 30_000;
const foundersPlotScheduler = {
  active: false,
...
async function runFoundersPlotSchedulerDueCheck() {
  ...
  try {
    ...
    const task =
      observationPayload?.observation?.scheduler?.collectReadyOutputs ||
      observationPayload?.scheduler?.collectReadyOutputs ||
      null;

    if (!task?.enabled || task.paused === true) {
      foundersPlotScheduler.lastStatus = "WAITING";
      scheduleFoundersPlotDueCheck(5_000);
      return;
    }
    ...
    if (!hasActionableCandidate) {
      ...
      scheduleFoundersPlotDueCheck(15_000);
      return;
    }
    ...
    const retryAfterMs = Number(
      error?.payload?.error?.details?.retryAfterMs ||
      error?.payload?.details?.retryAfterMs ||
      0
    );
    const delayMs = code === "RATE_LIMITED" && retryAfterMs > 0
      ? retryAfterMs
      : Math.min(30_000, 5_000 * foundersPlotScheduler.consecutiveErrors);
    postFoundersPlotSchedulerTick("worker.foundersPlot.scheduler.tick.failed", {
      taskKind: foundersPlotScheduler.taskKind,
      error: code,
...
self.addEventListener("message", async (ev) => {
  ...
  try {
    ...
    if (msg.type === "gateway.command.tools.invoke") {
      ...
      const result = await dispatchLiteTool(toolName, msg.params || {}, undefined, undefined);
      post({
