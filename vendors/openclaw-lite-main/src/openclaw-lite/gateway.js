...
async function init() {
  ...
  function buildDefaultToolsMd(toolNames = []) {
    const names = Array.isArray(toolNames)
      ? toolNames.map((name) => String(name || "").trim()).filter(Boolean).sort()
      : [];
    const lines = [
      "# Tools",
      "",
  ...
  async function ensureDefaultSkillPackCompiled({ force = false, idempotencyKey = "" } = {}) {
    ...
    compilePromise = (async () => {
      ...
      const activeSkillPath =
        typeof skillState?.activeSkillPath === "string" ? skillState.activeSkillPath.trim() : "";
      if (!activeSkillPath) {
      ...
      const toolNames = Array.isArray(toolRegistry?.names)
        ? toolRegistry.names.map((name) => String(name || "").trim()).filter(Boolean)
        : [];
      ...
      if (
        !force
        && defaultCompiledPackCache?.manifest?.contentHash === manifest.contentHash
        && defaultCompiledPackCache?.manifest?.packVersionId === manifest.packVersionId
      ) {
...
`,
  ...
  // Public runtime surface for page scripts (not test-only).
