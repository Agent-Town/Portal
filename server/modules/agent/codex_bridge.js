...
function clampBody(rawBody, maxBytes = 200_000) {
  ...
  if (Buffer.byteLength(body, 'utf8') > maxBytes) throw new Error('BODY_TOO_LARGE');
  return body;
}

function proxyViaCodexCli(req, res) {
  if (!isLocalRequest(req)) return res.status(403).json({ ok: false, error: 'LOCALHOST_ONLY' });

  const accessToken = readCodexAccessToken();
  if (!accessToken) return res.status(400).json({ ok: false, error: 'MISSING_CODEX_CLI_SESSION' });
  ...
  try {
    ...
    if (msg === 'BODY_TOO_LARGE') return res.status(413).json({ ok: false, error: 'BODY_TOO_LARGE' });
